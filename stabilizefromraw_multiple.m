...
flipVid = false; %setflipVid to true if AOSLO III and first run, otherwise false and select _r vids (rotated)

blinkthreshold = 50;                % This is a maximum change in mean pixel value between
                                    % frames before the function tags the frames as blink
                                    % frames.
minimummeanlevel = 66;              % This is minimum mean pixel value that a frame has to
...
numframestoaverage = -1;            % The number of frame to average toget if you want to remove the influence of
                                    ...
                                    % the frames
peakratiodiff = 0.4;   %from 0.05            % Maximum Change in ratio between the secondpeak and firstpeak
                                    % between two frames for the frames before the function tags the frames as "bad".
maxmotionthreshold = 0.4;           % Maximum motion between frames, expressed as a percentage
...
for filecounter = 1:numfiletoanalyse
    try
        ...
        if(flipVid)
            currentvideoinfo = rotateVideo(videotoanalyse);
            videotoanalyse = [videotoanalyse(1:end-4) '_r.avi']
        end
        ...
        switch formatofstabframe
            ...
            case '.tiff'
                ...
                if(flipVid || (videotoanalyse(end-5:end)=="_r.avi"))
                   imwrite(rot90(stabilizedmatrix(:,:,3),3) / 256,stabimagename,'Compression','none'); 
                else
